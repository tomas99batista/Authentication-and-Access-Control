...
class ClientProtocol(asyncio.Protocol):
	...
	def auth_server(self, message):
		...
		server_cert_pub_key_bytes = base64.b64decode(message['pub_key_cert'])
		# Key publica do certificado do servidor
		self.server_cert_pub_key = serialization.load_der_public_key(server_cert_pub_key_bytes, backend=default_backend())
		# Calculado
		hasher = hashes.Hash(hashes.SHA256(), default_backend())
		...
		# * Valida: common name, data de expiração, purpose e estado de revogação

		# ! Se tudo correu bem pede challenge_request // CC AUTH, depois depende
		# TODO: TOMAS
		# if modo a usar é challenge: else: CC_AUTH
		# self.challenge_request_password()
		self.challenge_request_cc()
	...
	def challenge_request_cc(self):
		txt = str.encode(json.dumps({'type': 'CHALLENGE_REQUEST_CC' })) # 'AQUI VAI ALGO': base64.b64encode().decode('utf-8')
		payload, mac = self.encript(txt)
		msg = {
			'type': 'SECURE_MESSAGE',
			'payload': base64.b64encode(payload).decode('utf-8'),
